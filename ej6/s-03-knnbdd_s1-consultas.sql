...
--@Descripción:     Creación de ligas para comunicación bidireccional S1

-- conectando a knnbdd_s1
connect bancos_bdd/bancos_bdd@knnbdd_s1

-- Realizando conteo de registros
set serveroutput on

--La consulta se lanza en KNNBDD_S1
declare
  v_num_paises number;
  v_num_bancos number;
...
begin
  ...
  from (
    ...
    union all
    select pais_id
    from f_knn_pais_2@knnbdd_s2.fi.unam
  ) q1;
  ...
  from (
    ...
    union all
    select banco_id
    from f_knn_banco_2@knnbdd_s2.fi.unam
  ) q2;
  ...
  from (
    select sucursal_id
    from f_knn_sucursal_1
    union all
    select sucursal_id
    from f_knn_sucursal_2@knnbdd_s2.fi.unam
  ) q3;
  ...
  from (
    select empleado_id
    from f_knn_empleado_1
    union all
    select empleado_id
    from f_knn_empleado_2@knnbdd_s2.fi.unam
  ) q4;
  ...
  from (
    ...
    union all
    select cuenta_id
    from f_knn_cuenta_4@knnbdd_s2.fi.unam
  ) q5;
  ...
  from (
    ...
    union all
    select num_movimiento
    from f_knn_movimiento_1@knnbdd_s2.fi.unam
    union all
    select num_movimiento
    from f_knn_movimiento_3@knnbdd_s2.fi.unam
  ) q6;
